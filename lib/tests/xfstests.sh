...
run_smbv2_tests()
{
	...
	[[ "$test" == "generic-751" ]] ||
		echo "generic/751" >>tests/exclude/smbv2
	# generic/755 checks that ctime updates when a hardlink is removed;
	# Samba derives ctime from mtime and never updates it on unlink, so
	# this always fails against a Samba server regardless of client fix
	echo "generic/755" >>tests/exclude/smbv2
	# generic/478 runs over an hour
	echo "generic/478" >>tests/exclude/smbv2
	log_cmd ./check -E tests/exclude/smbv2 $exclude_file $all_tests
}
...
run_smbv3_tests()
{
	...
	echo "generic/013" >>tests/exclude/smbv3
	log_cmd ./check -E tests/exclude/smbv3 $exclude_file $all_tests
}
